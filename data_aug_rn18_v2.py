...
if (K.image_data_format() == 'channels_first'):
    csv_logger = CSV_Logger('train_cifar10_resnet18_basic_th_data_aug_2.log', append=True)
else:
    csv_logger = CSV_Logger('train_cifar10_resnet18_basic_tf_data_aug_2.log', append=True)
...
def gen_with_norm(gen):
    for x_batch, y_batch in gen:
        if (K.image_data_format() == 'channels_first'):
            ...
            for i in range(x_batch.shape[3]):
                x_batch[:,:,:,i] -= mean
                x_batch[:,:,:,i] /= std_dev
...
for num_epochs, lr_rate in [(150, 0.1), (100, 0.01), (100, 0.001)]:
    ...
    history = model.fit_generator(gen_with_norm(datagen.flow(x_train, y_train, 
...
if (K.image_data_format() == 'channels_first'):
    model.save('cifar10-resnet18-basic-th-data-aug-2.h5')
else:
    model.save('cifar10-resnet18-basic-tf-data-aug-2.h5')
