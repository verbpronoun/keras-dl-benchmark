...
if (K.image_data_format() == 'channels_first'):
    csv_logger = CSV_Logger('train_cifar10_resnet18_basic_th_data_aug.log', append=True)
else:
    csv_logger = CSV_Logger('train_cifar10_resnet18_basic_tf_data_aug.log', append=True)
        ...
        for x_batch, y_batch in datagen.flow(x_train, y_train, batch_size=batch_size):
            if (K.image_data_format() == 'channels_first'):
                ...
                for i in range(x_train.shape[3]):
                    x_batch[:,:,:,i] -= mean
                    x_batch[:,:,:,i] /= std_dev
            x_batch_lists.append(x_batch)
            y_batch_lists.append(y_batch)
            batches += 1
            if batches >= len(x_train) / batch_size:
                # we need to break the loop by hand because
                # the generator loops indefinitely
                break
        x_train_concatenated = np.concatenate(x_batch_lists)
        y_train_concatenated = np.concatenate(y_batch_lists)

        history = model.fit(x_train_concatenated, y_train_concatenated, batch_size=batch_size,
                            epochs=1,
...
if (K.image_data_format() == 'channels_first'):
    model.save('cifar10-resnet18-basic-th-data-aug.h5')
else:
    model.save('cifar10-resnet18-basic-tf-data-aug.h5')
